# Termos de Uso — ApT (Áudio para Texto)

**Versão:** 1.0
**Vigência:** Março de 2026

---

## 1. Aceitação dos Termos

Ao instalar, copiar ou utilizar o software **ApT — Áudio para Texto** ("Software"), você concorda com estes Termos de Uso. Caso não concorde, não utilize o Software.

---

## 2. Licença de Uso — Versão Comunitária

A versão comunitária do ApT é distribuída sob a **Apache License, Version 2.0** ("Licença"). Você pode:

- Usar o Software para fins pessoais, educacionais e institucionais
- Copiar e redistribuir o Software, desde que mantenha os avisos de atribuição exigidos pela Licença
- Modificar o Software, desde que as modificações sejam claramente indicadas

A íntegra da Licença está disponível no arquivo `LICENSE` neste repositório e em [https://www.apache.org/licenses/LICENSE-2.0](https://www.apache.org/licenses/LICENSE-2.0).

---

## 3. Processamento Local e Privacidade

- Todo o processamento de áudio e geração de laudos é realizado **100% localmente** na máquina do usuário
- Nenhum arquivo de áudio, transcrição ou dado pessoal é transmitido para servidores externos
- O Software não coleta telemetria, não realiza chamadas de rede durante o uso e não armazena dados em nuvem

---

## 4. Responsabilidade pelo Uso Forense

O ApT é uma **ferramenta de auxílio técnico**. A responsabilidade pela validade, acurácia e adequação jurídica dos laudos produzidos é exclusivamente do profissional responsável pela operação, conforme:

- Art. 159 do Código de Processo Penal
- Arts. 158-A a 158-F do CPP (Lei 13.964/2019)
- ABNT NBR ISO/IEC 27037:2013

O autor do Software não assume responsabilidade por laudos utilizados em processos judiciais sem revisão e assinatura de profissional habilitado.

---

## 5. Ausência de Garantias

O Software é fornecido "no estado em que se encontra" (*as is*), sem garantias de qualquer natureza, expressas ou implícitas, incluindo, sem limitação, garantias de comercialização, adequação a uma finalidade específica ou não violação.

---

## 6. Versão Licenciada (Premium)

O uso da versão licenciada do ApT está sujeito ao **Contrato de Licença de Usuário Final (EULA)**, disponível no arquivo `EULA.md`. A versão licenciada requer a aquisição de uma licença individual ou institucional válida.

---

## 7. Contato

Dúvidas sobre estes termos: [https://www.brunnoml.com.br/apt](https://www.brunnoml.com.br/apt)
